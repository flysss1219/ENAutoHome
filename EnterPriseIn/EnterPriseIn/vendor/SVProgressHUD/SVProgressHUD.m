...
@property (nonatomic, strong) UIImageView *firstHouseImageView;
...
//        NSURL *url = [bundle URLForResource:@"SVProgressHUD" withExtension:@"bundle"];
        NSURL *bundlePath = [[NSBundle mainBundle] URLForResource:@"SVProgressHUD" withExtension:@"bundle"];

        NSBundle *imageBundle = [NSBundle bundleWithURL:bundlePath];
...
//    }
    ...
    self.hudView.bounds = CGRectMake(0.0f, 0.0f, 104, 104);
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    if (self.containerView) {
        self.hudView.center = CGPointMake(self.containerView.center.x + self.offsetFromCenter.horizontal, self.containerView.center.y + self.offsetFromCenter.vertical - 32);
    } else {
        self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + self.offsetFromCenter.vertical - 32);
    }
}
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            if(progress >= 0) {
                ...
                strongSelf.firstHouseImageView.frame = CGRectMake(strongSelf.indefiniteAnimatedView.frame.size.width/2 - 14, strongSelf.indefiniteAnimatedView.frame.size.height/2 - 14, 28, 28);
#else
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.statusLabel.hidden = status.length == 0;
            strongSelf.statusLabel.text = status;
            strongSelf.hudView.bounds = CGRectMake(0.0f, 0.0f, 104, 104);
...
//        NSLog(@"%f     %f",self.ringRadius,self.ringNoTextRadius);
...
//#endif
    _controlView.frame = CGRectMake(0, 64, ScreenSize.width, ScreenSize.height - 64);
    return _controlView;
}
...
- (UIView*)hudView {
    ...
    _hudView.bounds = CGRectMake(0.0f, 0.0f, 104, 104);
    // Update styling
...
- (UIImageView *)firstHouseImageView{
    if (!_firstHouseImageView) {
        _firstHouseImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 28, 28)];
        _firstHouseImageView.image = [UIImage imageNamed:@"pop_loading_no1"];
        if(!_firstHouseImageView.superview) {
            [self.indefiniteAnimatedView addSubview:self.firstHouseImageView];
        }
    }
    return _firstHouseImageView;
