...
{
    /**
     *  有时会出现提示信息不出现的情况，弹窗被加载到视图下面了
     *
     *  @param view change by wyy
     *
...
+ (void)showDetail:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
    // 快速显示一个提示信息
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view animated:YES];
...
{
    [self showDetail:message icon:nil view:view];
}
@end
