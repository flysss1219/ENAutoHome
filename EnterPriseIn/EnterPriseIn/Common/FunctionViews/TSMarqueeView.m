...
@interface UIView (TSViewExtension)
...
@implementation UIView (TSViewExtension)
// getter
- (CGFloat)lmj_top{
...
@interface UILabel (TSLabelExtension)

@property (nonatomic) id lmj_text;

@end

@implementation UILabel (TSLabelExtension)
- (id)lmj_text{
    return self.text;
}
...
- (void)clickAction{
    if ([self isCurrentViewControllerVisible:[self viewController]] && self.delegate && [self.delegate respondsToSelector:@selector(tsMarquee:clickIndex:content:)]) {
        [self.delegate tsMarquee:self clickIndex:_index content:[_textDataArr[_index] copy]];
    }
}
...
- (void)scrollWithNoSpaceByDirection:(NSNumber *)direction{
    ...
    if (![self isCurrentViewControllerVisible:[self viewController]]) {
        ...
        if ([self isCurrentViewControllerVisible:[self viewController]] && self.delegate && [self.delegate respondsToSelector:@selector(tsMarquee:currentTextIndex:)]) { // 代理回调
            [self.delegate tsMarquee:self currentTextIndex:_index];
        }
